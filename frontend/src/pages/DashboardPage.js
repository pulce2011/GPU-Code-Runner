...
function DashboardPage() {
  ...
  const handleCreditsUpdate = (newCredits) => {
    setUserInfo(prev => ({
      ...prev,
      credits: typeof newCredits === 'function' ? newCredits(prev.credits) : newCredits
    }));
  };
  ...
  const getStatusClass = (status) => {
    const statusMap = {
      ...
      insufficient_credits: 'bg-red-100 text-red-800'
    };
  ...
  const getStatusText = (status) => {
    const statusMap = {
      ...
      pending: 'In attesa',
      insufficient_credits: 'Crediti esauriti'
    };
            ...
            <div className="flex justify-between items-start">
              <div>
                <h2 className="text-3xl font-semibold text-gray-900 mb-2">
                  Benvenuto, {userInfo.last_name} {userInfo.first_name} 
                  <span className="text-sm"> [MATR. {userInfo.matr}]</span>
                ...
                <div 
                  ...
                  <div className="flex items-center">
                    <div className={`flex-shrink-0 mr-3 ${
                      ...
                      taskDetails.status === 'insufficient_credits' ? 'text-red-500' :
                      'text-gray-500'
                    }`}>
                      {getStatusIcon(taskDetails.status)}
                    </div>
                    <div className={`text-sm font-medium ${
                      taskDetails.status === 'completed' ? 'text-green-800' :
                      ...
                      taskDetails.status === 'insufficient_credits' ? 'text-red-800' :
                      'text-gray-800'
                    }`}>
                      {taskDetails.message}
                    </div>
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <span className={`ml-2 px-2 py-1 rounded text-xs font-medium ${getStatusClass(taskDetails.status)}`}>
